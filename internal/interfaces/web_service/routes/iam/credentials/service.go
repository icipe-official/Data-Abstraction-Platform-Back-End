...
func (n *service) ServiceIamCredentialsSearch(
	...
	if value, err := n.repo.RepoIamCredentialsSearch(
		ctx,
		mmsearch,
		repo,
		iamCredential,
		...
		n.logger.Log(ctx, slog.LevelError, intlib.FunctionNameAndError(n.ServiceIamCredentialsSearch, err).Error())
...
func (n *service) ServiceIamCredentialsGetMetadataModel(ctx context.Context, metadataModelRetrieve intdomint.MetadataModelRetrieve, targetJoinDepth int) (map[string]any, error) {
	if value, err := metadataModelRetrieve.IamCredentialsGetMetadataModel(ctx, 0, targetJoinDepth, nil); err != nil {
		n.logger.Log(ctx, slog.LevelWarn+1, intlib.FunctionNameAndError(n.ServiceIamCredentialsGetMetadataModel, err).Error())
		return nil, intlib.NewError(http.StatusInternalServerError, http.StatusText(http.StatusInternalServerError))
	} else {
		return value, nil
	}
