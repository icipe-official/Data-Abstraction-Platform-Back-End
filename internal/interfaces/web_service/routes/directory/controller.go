...
func ApiCoreRouter(webService *intwebservice.WebService) *chi.Mux {
	...
	router.Post("/delete", func(w http.ResponseWriter, r *http.Request) {
		...
		data := make([]*intdoment.Directory, 0)
		...
		if code, verbres, err := s.ServiceDirectoryDeleteMany(
			ctx,
			authedIamCredential,
			nil,
			...
			webService.Logger.Log(
				ctx,
				slog.LevelInfo+1,
				intlib.LogAction(intlib.LOG_ACTION_DELETE, intdoment.DirectoryRepository().RepositoryName),
	...
	router.Post("/update", func(w http.ResponseWriter, r *http.Request) {
		...
		if code, verbres, err := s.ServiceDirectoryUpdateMany(
			ctx,
			authedIamCredential,
			nil,
			authContextDirectoryGroupID,
			...
			webService.Logger.Log(
				ctx,
				slog.LevelInfo+1,
				intlib.LogAction(intlib.LOG_ACTION_UPDATE, intdoment.DirectoryRepository().RepositoryName),
				ctx.Value(intlib.LOG_ATTR_CTX_KEY),
				"authenicated iam credential",
				intlib.JsonStringifyMust(authedIamCredential),
				"verbose response data",
	...
	router.Post("/create", func(w http.ResponseWriter, r *http.Request) {
		...
		if code, verbres, err := s.ServiceDirectoryInsertMany(
			ctx,
			authedIamCredential,
			nil,
			authContextDirectoryGroupID,
			...
			webService.Logger.Log(
				ctx,
				slog.LevelInfo+1,
				intlib.LogAction(intlib.LOG_ACTION_CREATE, intdoment.DirectoryRepository().RepositoryName),
				ctx.Value(intlib.LOG_ATTR_CTX_KEY),
				"authenicated iam credential",
				intlib.JsonStringifyMust(authedIamCredential),
				"verbose response data",
	...
	router.Route("/search", func(searchRouter chi.Router) {
		searchRouter.Post("/", func(w http.ResponseWriter, r *http.Request) {
			...
			if mmSearch.MetadataModel == nil {
				if value, err := s.ServiceDirectoryGetMetadataModel(
					ctx,
			...
			if searchResults, err := s.ServiceDirectorySearch(
				ctx,
				mmSearch,
...
func initApiCoreService(ctx context.Context, webService *intwebservice.WebService) intdomint.RouteDirectoryApiCoreService {
