...
import (
	...
	intlibexport "github.com/icipe-official/Data-Abstraction-Platform-Back-End/internal/lib/export"
...
func ApiCoreRouter(webService *intwebservice.WebService) *chi.Mux {
	...
	router.Post("/export", func(w http.ResponseWriter, r *http.Request) {
		...
		data := new(intdoment.MetadataModelSearch)
		...
		searchResults, err := s.ServiceAbstractionsSearch(
			ctx,
			data,
		...
		searchResults.MetadataModel = intlibmmodel.MapFieldGroups(searchResults.MetadataModel, func(property map[string]any) any {
			delete(property, intlibmmodel.FIELD_GROUP_PROP_GROUP_EXTRACT_AS_SINGLE_FIELD)
		...
		if storageFiles, err := intlibexport.ExportToCSV(ctx, searchResults, webService.PostgresRepository, webService.FileService, intdoment.AbstractionsRepository().RepositoryName); err != nil {
			...
			intlib.SendJsonResponse(http.StatusOK, storageFiles, w)
...
func initApiCoreService(ctx context.Context, webService *intwebservice.WebService) intdomint.RouteAbstractionsApiCoreService {
