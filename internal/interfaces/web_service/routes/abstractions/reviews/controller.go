...
func ApiCoreRouter(webService *intwebservice.WebService) *chi.Mux {
	...
	router.Post("/upsert", func(w http.ResponseWriter, r *http.Request) {
		...
		data := make([]*intdoment.AbstractionsReviews, 0)
		...
		if code, verbres, err := s.ServiceAbstractionsReviewsUpsertMany(
			ctx,
			authedIamCredential,
			nil,
			...
			webService.Logger.Log(
				ctx,
				slog.LevelInfo+1,
				intlib.LogAction(intlib.LOG_ACTION_CREATE, intdoment.AbstractionsReviewsRepository().RepositoryName),
	...
	router.Route("/search", func(searchRouter chi.Router) {
		searchRouter.Post("/", func(w http.ResponseWriter, r *http.Request) {
			...
			if mmSearch.MetadataModel == nil {
				if value, err := s.ServiceAbstractionsReviewsGetMetadataModel(
					ctx,
			...
			if searchResults, err := s.ServiceAbstractionsReviewsSearch(
				ctx,
				mmSearch,
				...
				webService.Logger.Log(
					...
					slog.LevelInfo+1,
					fmt.Sprintln(
						intdoment.AbstractionsReviewsRepository().RepositoryName,
...
func initApiCoreService(ctx context.Context, webService *intwebservice.WebService) intdomint.RouteAbstractionsReviewsApiCoreService {
