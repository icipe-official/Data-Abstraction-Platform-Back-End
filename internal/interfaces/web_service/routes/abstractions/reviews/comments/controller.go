...
func ApiCoreRouter(webService *intwebservice.WebService) *chi.Mux {
	...
	router.Post("/create", func(w http.ResponseWriter, r *http.Request) {
		...
		data := make([]*intdoment.AbstractionsReviewsComments, 0)
		...
		if code, verbres, err := s.ServiceAbstractionsReviewsCommentsInsertMany(
			ctx,
			authedIamCredential,
			nil,
			...
			webService.Logger.Log(
				ctx,
				slog.LevelInfo+1,
				intlib.LogAction(intlib.LOG_ACTION_CREATE, intdoment.AbstractionsReviewsCommentsRepository().RepositoryName),
	...
	router.Route("/search", func(searchRouter chi.Router) {
		searchRouter.Post("/", func(w http.ResponseWriter, r *http.Request) {
			...
			if mmSearch.MetadataModel == nil {
				if value, err := s.ServiceAbstractionsReviewsCommentsGetMetadataModel(
					ctx,
			...
			if searchResults, err := s.ServiceAbstractionsReviewsCommentsSearch(
				ctx,
				mmSearch,
				...
				webService.Logger.Log(
					...
					slog.LevelInfo+1,
					fmt.Sprintln(
						intdoment.AbstractionsReviewsCommentsRepository().RepositoryName,
...
func initApiCoreService(ctx context.Context, webService *intwebservice.WebService) intdomint.RouteAbstractionsReviewsCommentsApiCoreService {
