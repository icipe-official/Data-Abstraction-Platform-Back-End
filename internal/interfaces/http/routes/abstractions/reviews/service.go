...
func (n *service) ServiceAbstractionsReviewsUpsertMany(
	ctx context.Context,
	...
	if verboseResponse {
		if d, err := intlib.MetadataModelMiscGet(intlib.METADATA_MODELS_MISC_VERBOSE_RESPONSE); err != nil {
			n.logger.Log(ctx, slog.LevelError, intlib.FunctionNameAndError(n.ServiceAbstractionsReviewsUpsertMany, err).Error())
	...
	for _, datum := range data {
		...
		if len(datum.AbstractionsID) > 0 && len(datum.ReviewPass) > 0 {
			...
			if value, err := n.repo.RepoAbstractionsReviewsInsertOne(ctx, datum, nil); err != nil {
				if err2 := n.repo.RepoAbstractionsReviewsUpdateOne(ctx, datum, iamCredential, iamAuthorizationRules, authContextDirectoryGroupID); err2 != nil {
					n.logger.Log(ctx, slog.LevelError, intlib.FunctionNameAndError(n.ServiceAbstractionsReviewsUpsertMany, err).Error())
					verbRes.Data = []any{datum}
					verbRes.Status = make([]intdoment.MetadataModelVerboseResponseStatus, 1)
					verbRes.Status[0].StatusCode = []int{http.StatusInternalServerError}
					verbRes.Status[0].StatusMessage = []string{http.StatusText(http.StatusInternalServerError), "upsert failed", err.Error(), err2.Error()}
					failed += 1
					goto appendNewVerboseResponse
				} else {
					verbRes.Data = []any{datum}
					...
					verbRes.Status[0].StatusMessage = []string{http.StatusText(http.StatusOK), "upsert successful"}
					successful += 1
					goto appendNewVerboseResponse
				}
			} else {
	...
	verbres.Message = fmt.Sprintf("Upsert %[1]s: %[2]d/%[4]d successful and %[3]d/%[4]d failed", intdoment.AbstractionsReviewsRepository().RepositoryName, successful, failed, len(data))
...
func (n *service) ServiceAbstractionsReviewsSearch(
	ctx context.Context,
	...
	if value, err := n.repo.RepoAbstractionsReviewsSearch(
		ctx,
		mmsearch,
		repo,
		iamCredential,
		...
		n.logger.Log(ctx, slog.LevelError, intlib.FunctionNameAndError(n.ServiceAbstractionsReviewsSearch, err).Error())
...
func (n *service) ServiceAbstractionsReviewsGetMetadataModel(ctx context.Context, metadataModelRetrieve intdomint.MetadataModelRetrieve, targetJoinDepth int) (map[string]any, error) {
	if value, err := metadataModelRetrieve.AbstractionsReviewsGetMetadataModel(ctx, 0, targetJoinDepth, nil); err != nil {
		n.logger.Log(ctx, slog.LevelWarn+1, intlib.FunctionNameAndError(n.ServiceAbstractionsReviewsGetMetadataModel, err).Error())
		return nil, intlib.NewError(http.StatusInternalServerError, http.StatusText(http.StatusInternalServerError))
	} else {
		return value, nil
	}
...
type service struct {
	repo   intdomint.RouteAbstractionsReviewsRepository
